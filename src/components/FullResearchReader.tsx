...
export default function FullResearchReader({document,onClose,onRequest}:Props){
  ...
  return <motion.div className='reader-overlay' role='presentation' onClick={onClose} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
    <motion.section className='reader-shell' role='dialog' aria-modal='true' aria-label={`معاينة البحث الكامل: ${enhancedDocument.title}`} onClick={e=>e.stopPropagation()} initial={{opacity:0,y:24,scale:.985}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:18,scale:.99}}>
      <header className='reader-topbar'>
        <div className='reader-title'><span className='reader-brand'>مَرجِع</span><div><small>{enhancedDocument.studyType} • {enhancedDocument.standard}</small><strong>{enhancedDocument.title}</strong></div></div>
      ...
      <div className='reader-layout'>
        <aside className='reader-sidebar'>
          ...
          <div className='reader-integrity'><ShieldCheck size={17}/><p><b>بحث عرض أكاديمي احترافي.</b> النتائج الرقمية لا تُعرض إلا من بيانات حقيقية أو مصدر موثق.</p></div>
        </aside>
        ...
        <main className='reader-stage'>
          ...
          <div className='paper-viewport'>
            <AnimatePresence mode='wait'>
              <motion.article key={`${enhancedDocument.id}-${page}`} className={`research-paper ${current.number===1?'cover-page':''}`} style={{transform:`scale(${zoom/100})`}} initial={{opacity:0,x:-22}} animate={{opacity:1,x:0}} exit={{opacity:0,x:22}} transition={{duration:.22}}>
                <div className='paper-watermark'>MARJA SAMPLE</div>
                {current.number===1?<>
                  <div className='cover-mark'><span>م</span><b>مَرجِع</b></div>
                  <div className='cover-rule'/><span className='paper-kicker'>{current.kicker}</span><h1>{current.title}</h1><p className='cover-sub'>بحث عرض أكاديمي احترافي كامل • نسخة تفاعلية</p>
                  <div className='cover-meta'><span>Professional Research Sample</span><span>2026</span><span>{enhancedDocument.standard}</span></div>
                </>:<>
                ...
                <footer className='paper-page-footer'><span>مَرجِع • Professional Academic Sample</span><b>{current.number} / {enhancedDocument.pageCount}</b></footer>
              </motion.article>
            </AnimatePresence>
          </div>
