    ...
    <MotionConfig reducedMotion='user' transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}>
      <main className='site'>
        ...
        <section className='library-section' id='work'>
          <div className='wrap'>
            <motion.div className='section-head library-head' initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <div><span className='eyebrow'>مكتبة الأبحاث</span><h2>لا تشترِ وعدًا.<br />افتح الملف وشوف البناء.</h2></div>
              <p>نماذج عرض تعليمية مبنية على موضوعات ومراجع ومنهجيات حقيقية. النتائج الرقمية الميدانية لا تُختلق؛ تظهر كقالب حتى تصل البيانات.</p>
            </motion.div>
            <div className='library-toolbar'><div className='portfolio-tabs'>{categories.map(({ key, label, icon: Icon }) => <button key={key} type='button' className={portfolioFilter === key ? 'active' : ''} onClick={() => setPortfolioFilter(key)}><Icon size={14} />{label}</button>)}</div><span>{filteredShowcase.length} ملفات قابلة للتصفح</span></div>
            <motion.div layout className='research-grid'>
              {filteredShowcase.map((item, index) => (
                <motion.article layout key={item.id} className='research-card' initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .035, .22) }}>
                  <div className='research-card-head'><span>{item.signal}</span><b>{item.standard}</b></div>
                  <div className='card-file-preview'><div className='file-tab'>{item.studyType}</div><div className='file-title'>{item.title}</div><div className='file-lines'><i /><i /><i /><i /></div><div className='file-seal'><ShieldCheck size={13} /><small>METHOD<br />READY</small></div></div>
                  <div className='card-kicker'><span>{item.studyType}</span><span>18 صفحة</span><span>نسخة عرض</span></div>
                  <h3>{item.title}</h3>
                  <p>{item.problem}</p>
                  <div className='card-proof'><small>ما الذي تم بناؤه؟</small><b>{item.outcome}</b><div>{item.deliverables.slice(0, 3).map((deliverable) => <span key={deliverable}><Check size={12} />{deliverable}</span>)}</div></div>
                  <div className='card-tags'>{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
                  <footer><button type='button' className='preview-btn' onClick={() => setActiveSample(item)}><Eye size={14} /> افتح المعاينة</button><button type='button' className='request-btn' onClick={() => requestLike(item.service)}>أبغى مثله <ArrowLeft size={14} /></button></footer>
                </motion.article>
              ))}
            </motion.div>
            <div className='integrity-note'><ShieldCheck size={19} /><p><b>واضحين في نماذج العرض:</b> هذه الملفات التعليمية تعرض طريقة التفكير والتسليم، وليست أعمال عملاء أو نتائج ميدانية منسوبة لأحد. المراجع المختارة حقيقية، والأرقام لا تُملأ إلا بعد توفر البيانات.</p></div>
          </div>
        </section>
