...
import { ArrowLeft, BarChart3, BookOpenCheck, BriefcaseBusiness, Check, CheckCircle2, ChevronDown, Clock3, Eye, FileSearch, FileText, GraduationCap, Laptop2, Microscope, Presentation, School, Search, Send, ShieldCheck, Sparkles, Stethoscope, Users2, X } from 'lucide-react';
...
function App() {
  ...
  return <MotionConfig reducedMotion='user' transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}><main className='site'>
    ...
    <section className='portfolio-section' id='work'><div className='wrap'>
      <motion.div className='section-head work-head' initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><div><span className='eyebrow'>كتالوج الأعمال</span><h2>نماذج تخليك تعرف المستوى قبل ما تطلب.</h2></div><p>نفصل بوضوح بين المشروع الموثّق والنماذج التي بنيناها للعرض. كل نموذج يوضح نوع البحث، المخرجات، والمنهجية بدون ادعاء عميل أو نتيجة غير موثقة.</p></motion.div>
    ...
    <AnimatePresence>{activeSample && <motion.div className='modal-backdrop' role='presentation' onClick={() => setActiveSample(null)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.div className='sample-modal' role='dialog' aria-modal='true' aria-label={`معاينة ${activeSample.title}`} onClick={(event) => event.stopPropagation()} initial={{ opacity: 0, y: 28, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.98 }}><button className='modal-close' aria-label='إغلاق المعاينة' onClick={() => setActiveSample(null)}><X size={19}/></button><div className='modal-copy'><span className='sample-label'>نموذج عرض من تنفيذنا • {activeSample.standard}</span><h2>{activeSample.title}</h2><p>{activeSample.subtitle}</p><div className='modal-block'><small>المشكلة</small><strong>{activeSample.problem}</strong></div><div className='modal-block result'><small>ما الذي بنيناه</small><strong>{activeSample.outcome}</strong></div><div className='deliverables'><small>المخرجات</small>{activeSample.deliverables.map((item) => <span key={item}><Check size={14}/>{item}</span>)}</div><button className='btn accent' onClick={() => requestLike(activeSample.service)}>أبغى نموذج بنفس المستوى <ArrowLeft size={16}/></button></div><div className='modal-paper'><div className='paper-header'><Microscope size={18}/><div><small>{activeSample.studyType}</small><b>{activeSample.title}</b></div></div><div className='paper-section'><span>01</span><div><b>Research question</b><i/><i/></div></div><div className='paper-section'><span>02</span><div><b>Methodology</b><i/><i/><i/></div></div><div className='paper-section'><span>03</span><div><b>Analysis / Deliverables</b><i/><i/></div></div><div className='paper-footer'><span>{activeSample.standard}</span><span>مَرجِع • Sample</span></div></div></motion.div></motion.div>}</AnimatePresence>
  </main></MotionConfig>;
}
